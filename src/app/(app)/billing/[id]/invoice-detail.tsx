...
export function InvoiceDetail({ invoice, backUrl = '/billing' }: { invoice: any; backUrl?: string }) {
  ...
  const handleRevert = async () => {
    ...
    try {
      ...
      toast.success(to === 'draft' ? 'Factura revertida a borrador' : 'Pago revertido, factura marcada como enviada')
      router.refresh()
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {!editMode && (invoice.status === 'sent' || invoice.status === 'paid') && (
            <Button
              variant="ghost"
              size="sm"
              className="text-slate-400 hover:text-slate-600 text-xs gap-1 h-8"
              onClick={() => setShowRevertModal(true)}
              disabled={updating}
              title={invoice.status === 'paid' ? 'Revertir el pago' : 'Revertir a borrador'}
            >
